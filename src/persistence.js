...
function normalizeRobloxMonitorSubscriberAccounts(entries) {
  ...
  for (const [userId, account] of Object.entries(entries)) {
    ...
    const robloxUserId = Number.isInteger(account.robloxUserId) && account.robloxUserId > 0
      ...
      : null;
    const source = account.source === 'clan_auto'
      ? 'clan_auto'
      : (account.source === 'opt_in'
        ? 'opt_in'
        : (account.source === 'manual_opt_in_nick'
          ? 'manual_opt_in_nick'
...
function migrateClanState(state) {
  ...
  if ('clan_pending_workflows' in state) {
    delete state.clan_pending_workflows;
  }
